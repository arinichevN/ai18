...
#define PMEM_OWIRE_SIZE sizeof(AppOwireParam)
...
#define PMEM_OWIRE_ADDR(ind) 			PMEM_CRC_SIZE + PMEM_APP_SIZE + serials.length*(PMEM_CRC_SIZE + PMEM_SERIAL_SIZE) + ind*(PMEM_CRC_SIZE + PMEM_OWIRE_SIZE) + PMEM_CRC_SIZE
#define PMEM_OWIRE_CRC_ADDR(ind)		PMEM_CRC_SIZE + PMEM_APP_SIZE + serials.length*(PMEM_CRC_SIZE + PMEM_SERIAL_SIZE) + ind*(PMEM_CRC_SIZE + PMEM_OWIRE_SIZE)

#define PMEM_CHANNEL_ADDR(ind) 			PMEM_CRC_SIZE + PMEM_APP_SIZE + serials.length*(PMEM_CRC_SIZE + PMEM_SERIAL_SIZE) + owires.length*(PMEM_CRC_SIZE + PMEM_OWIRE_SIZE) + ind*(PMEM_CRC_SIZE + PMEM_CHANNEL_SIZE) + PMEM_CRC_SIZE
#define PMEM_CHANNEL_CRC_ADDR(ind)		PMEM_CRC_SIZE + PMEM_APP_SIZE + serials.length*(PMEM_CRC_SIZE + PMEM_SERIAL_SIZE) + owires.length*(PMEM_CRC_SIZE + PMEM_OWIRE_SIZE) + ind*(PMEM_CRC_SIZE + PMEM_CHANNEL_SIZE)

#define PMEM_CRC_INI 17

extern ChannelLList channels;
extern AppSerialLList serials;
extern AppOwireLList owires;
...
void pmem_setOwireCrc(const AppOwireParam *item, size_t ind){
	uint8_t crc = pmem_calcCrc(item, PMEM_OWIRE_SIZE);	
	//printd("   serial save crc: ");printdln(crc);
	EEPROM.put(PMEM_OWIRE_CRC_ADDR(ind), crc);
}

int pmem_checkSize(){
	size_t total_bytes = PMEM_APP_SIZE + PMEM_CRC_SIZE + serials.length*(PMEM_SERIAL_SIZE + PMEM_CRC_SIZE) + owires.length*(PMEM_OWIRE_SIZE + PMEM_CRC_SIZE) + channels.length*(PMEM_CHANNEL_SIZE + PMEM_CRC_SIZE) + 1;
...
int pmem_checkOwireCrc(const AppOwireParam *item, int ind){
	uint8_t crc1 = EEPROM[PMEM_OWIRE_CRC_ADDR(ind)];
	//printd("   serial get crc: ");printdln(crc1);
	uint8_t crc2 = pmem_calcCrc(item, PMEM_OWIRE_SIZE);
	if(crc1 != crc2){
		printd("   bad crc: ");printd(crc1);printd(" ");printdln(crc2);
		return 0;
	}
...
int pmem_hasSpaceForOwire(size_t ind){
	size_t required_bytes = PMEM_OWIRE_ADDR(ind) + PMEM_OWIRE_SIZE + PMEM_CRC_SIZE;
	if(EEPROM.length() < required_bytes){
		return 0;
	}
	return 1;
...
int pmem_getOwireParam(AppOwireParam *item, size_t ind) {
	if(!pmem_hasSpaceForOwire(ind)){
		return 0;
	}
	AppOwireParam t_item;
	EEPROM.get(PMEM_OWIRE_ADDR(ind), t_item);
	if(!pmem_checkOwireCrc(&t_item, ind)){
		return 0;
	}
	*item = t_item;
	return 1;
...
int pmem_saveOwireParam(const AppOwireParam *item, size_t ind) {
	//printd("(saving serial: ");printd(ind);
	if(!pmem_hasSpaceForOwire(ind)){
		printd("no space) ");
		return 0;
	}
	EEPROM.put(PMEM_OWIRE_ADDR(ind), *item);
	pmem_setOwireCrc(item, ind);
	printd("done) ");
	return 1;
}
