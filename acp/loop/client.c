...
int acplc_getBrII(ACPL *item, int cmd, int *v1, int *v2){
	...
	switch(item->state){
		...
		case ACP_DONE:
			...
			if(item->buf[1] != ACP_SIGN_RESPONSE){
				return ACP_ERROR_SIGN;
			}
			int _v1;
			if(!acp_packGetCellI (item->buf, 1, &_v1)){
				return ACP_ERROR_FORMAT;
			}
			if(!acp_packGetCellI (item->buf, 2, v2)){
				return ACP_ERROR_FORMAT;
			}
			*v1 = _v1;
			return ACP_DONE;
...
int acplc_getIS(ACPL *item, int cmd, int channel_id, char *out, size_t slen){
	switch(item->state){
		...
		case ACP_DONE:
			...
			int tchannel_id;
			if(!acp_packGetCellI (item->buf, 1, &tchannel_id)){
				return ACP_ERROR_FORMAT;
			}
			...
			if(!acp_packGetCellS (item->buf, 2, out, slen)){
				return ACP_ERROR_FORMAT;
			}
			return ACP_DONE;
		case ACP_IDLE:{
...
int acplc_getIF(ACPL *item, int cmd, int channel_id, double *out){
	switch(item->state){
		...
		case ACP_DONE:
			...
			if(!acp_packGetCellF (item->buf, 2, out)){
				return ACP_ERROR_FORMAT;
			}
			return ACP_DONE;
		case ACP_IDLE:{
...
int acplc_getIII(ACPL *item, int cmd, int channel_id, int id2, int *out){
	switch(item->state){
		...
		case ACP_DONE:
			...
			int tid2;
			if(!acp_packGetCellI (item->buf, 2, &tid2)){
				return ACP_ERROR_FORMAT;
			}
			if(tid2 != id2){
				printd(tid2);
			...
			if(!acp_packGetCellI (item->buf, 3, out)){
				return ACP_ERROR_FORMAT;
			}
			return ACP_DONE;
		case ACP_IDLE:{
...
int acplc_getIIII(ACPL *item, int cmd, int channel_id, int id2, int *v1, int *v2){
	switch(item->state){
		...
		case ACP_DONE:
			...
			int _v1;
			if(!acp_packGetCellI (item->buf, 3, &_v1)){
				return ACP_ERROR_FORMAT;
			}
			if(!acp_packGetCellI (item->buf, 4, v2)){
				return ACP_ERROR_FORMAT;
			}
			*v1 = _v1;
			return ACP_DONE;
...
int acplc_getIUl(ACPL *item, int cmd, int channel_id, unsigned long *out){
	switch(item->state){
		...
		case ACP_DONE:
			...
			if(!acp_packGetCellUl (item->buf, 2, out)){
				return ACP_ERROR_FORMAT;
			}
			return ACP_DONE;
		case ACP_IDLE:{
